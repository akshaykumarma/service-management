...
- [X] T021 [P] [US4] Contract test for `GET /api/tickets/:id/audit-trail` in `tests/contract/board-reporting-api.test.ts`
- [X] T022 [P] [US4] Integration test asserting the consolidated audit trail has zero gaps against what `003`-`005` independently recorded (SC-005) in `tests/integration/ticket-detail-audit.test.ts`. Runs a ticket through the real create → in_progress → line-item → completed → OTP-deliver → verify flow and confirms every source (`status_history`, `line_item`, `notification`, `otp`) appears in chronological order.

### Implementation for User Story 4

- [X] T023 [US4] Implement the consolidated audit-trail merge — `UNION ALL` across `status_history`, `ticket_line_items`, `notifications`, `otp_verifications`, and `delivery_overrides`, chronologically sorted, as a read-time view rather than a new stored table (`data-model.md`) — in `lib/audit/consolidated-trail.ts` (depends on T005). Implemented as five separate queries merged and sorted in application code rather than a literal SQL `UNION ALL` (the five sources have incompatible column shapes; a real `UNION ALL` would need per-source casting to a common shape anyway, so this is equivalent work with simpler code). **Real gap found and documented, not fixable from this feature alone**: `ticket_line_items` (004) records no actor at all — no `added_by` column exists — and a removed line item is hard-deleted with zero trace (004 never wrote a removal event anywhere). This means FR-012's "actor, timestamp, and what changed" and SC-005's "zero gaps... against what other specs recorded" both have a real, narrow exception: a part or service added then removed before a ticket reached Completed leaves no audit record whatsoever. This view can only surface what its source features actually captured; closing this gap would mean retroactively adding an actor column and a removal-event mechanism to `004`'s already-shipped schema, out of this feature's scope to decide unilaterally.
- [X] T024 [US4] Implement `GET /api/tickets/:id/audit-trail` in `app/api/tickets/[id]/audit-trail/route.ts` (depends on T023)
- [X] T025 [US4] Extend the ticket detail page with all required sections — Intake Info, Service History, Status Timeline, Parts & Services with Bill Summary, WhatsApp Notification Log, OTP Verification outcome, and Audit Trail (FR-011) — in `app/(dashboard)/tickets/[id]/page.tsx` (depends on T024). `GET /api/tickets/:id` gained three more additive fields (`serviceHistory`, `notificationLog`, `otpOutcome`), same pattern as `bill`/`notificationAlert`/`delivery` before it — `serviceHistory` reuses `003`'s own `lookupHistory()`, filtering out the ticket's own id (that function was written to check history *before* a ticket exists, so it doesn't already exclude the current one). Verified all four new sections render correctly in a real browser (`next build && next start`).
- [X] T026 [US4] Confirm T021-T022 pass; run `quickstart.md` Scenario 4 (depends on T023-T025). Passes; full suite (133 vitest tests, 10 e2e specs) green; `npx tsc --noEmit` clean.
...
- [X] T027 [P] [US5] Contract tests for `GET /api/reports/summary` and `GET /api/reports/export` (all documented responses, including `403` for Store Service Manager and `400 invalid_format`) in `tests/contract/board-reporting-api.test.ts`. 6 tests added (2 for `/summary`, 4 for `/export` covering CSV list export, PDF summary export, `invalid_format`, and the SM 403); all passed against the real routes on first run.
- [X] T028 [P] [US5] Integration test for summary aggregation — first-Completed period attribution (FR-017, SC-006), Open-to-Completed resolution time excluding pickup wait (FR-018, SC-007), and parts/services revenue split — in `tests/integration/summary-report.test.ts`. 3 tests, using the real `statusPATCH`/`partsPOST`/`servicesPOST`/`lineItemsPOST`/`summaryGET` routes and direct `status_history`/`tickets` timestamp backdating to test period-attribution independent of wall-clock time. **Bug found and fixed during writing**: the third test's catalogue-mutation calls (`partsPOST`/`servicesPOST`) initially used an Admin-role cookie, but those routes are Super-Admin-only (`requireSuperAdmin`) — added a dedicated `superAdmin`/`superAdminCookie` pair for just those two calls, matching the pattern already used in `store-setup.test.ts`.

### Implementation for User Story 5

- [X] T029 [US5] Implement the summary aggregation (total tickets, breakdown by current status, average resolution time, parts revenue, services revenue — joining `first-completed.ts` once per ticket per `data-model.md`) in `lib/reporting/summary.ts` (depends on T008). Single raw-SQL query with a `first_completed` CTE and `FILTER (WHERE ...)` conditional aggregation for parts vs. services revenue in one pass; JS-side loop aggregates `byStatus`/`avgResolutionTimeHours` from the returned rows.
- [X] T030 [US5] Implement CSV export (list export from `ticket-query.ts`'s results, or summary export from `summary.ts`'s aggregates) in `lib/reporting/csv-export.ts` (depends on T006, T029). Uses `csv-stringify/sync` (sibling of 004's already-used `csv-parse`); `ticketListToCsv` and `summaryToCsv`.
- [X] T031 [US5] Implement PDF export via `pdfkit`, matching the same two export shapes as T030 exactly (spec.md's own requirement: exported content matches what's shown on screen) in `lib/reporting/pdf-export.ts` (depends on T006, T029). `pdfkit` chosen per `research.md` (no headless browser, small-VPS friendly); stream-to-`Buffer` helper (`collectPdf`) shared by both export functions.
- [X] T032 [US5] Implement `GET /api/reports/summary` (Admin/Super-Admin-only, FR-015) in `app/api/reports/summary/route.ts` (depends on T029). `requireAdminOrAbove` + `assertAccess` on the requested `storeId`; `400 missing_required_field` for absent `storeId`/`dateFrom`/`dateTo`.
- [X] T033 [US5] Implement `GET /api/reports/export` (`format=csv|pdf` query param, list or summary export via `type=summary`, Admin/Super-Admin-only) in `app/api/reports/export/route.ts` (depends on T030, T031). `400 invalid_format` for anything other than `csv`/`pdf`; list export reuses `queryScopedTickets` with the same filter params `GET /api/tickets` accepts, so export visibility can never drift from board/filter visibility (research.md §2). Fixed a TS error wrapping `pdfkit`'s Node `Buffer` in `new Uint8Array(...)` for `NextResponse`, since the DOM `BodyInit` type doesn't accept `Buffer` directly.
- [X] T034 [US5] Build the reports UI (summary display, date-range/store selectors, CSV/PDF export controls) in `app/(dashboard)/reports/page.tsx` (depends on T032-T033). Store/date-range form (mirrors the board's filter-form pattern, no CSS per the rest of the app); on generate, shows totals/by-status breakdown/avg resolution/revenue and four export links (summary CSV/PDF, matching ticket-list CSV/PDF) built from the same filter params. Verified live in a real browser via Playwright (see T037): generating a report and downloading a CSV both work end-to-end.
- [X] T035 [US5] Confirm T027-T028 pass; run `quickstart.md` Scenario 5 (depends on T029-T034). All 12 tests in `board-reporting-api.test.ts` and all 3 in `summary-report.test.ts` pass. Scenario 5 manually re-verified via the T037 e2e test: a ticket created and Completed today appears in today's summary/export and not in an unrelated date range; RBAC 403 for Store Service Manager confirmed by T027's contract tests.
...
- [X] T036 [P] Run the full `quickstart.md` validation (all 5 scenarios, all 8 success-criteria checklist items) end-to-end. All 5 scenarios re-verified across this session's tests: Scenario 1 (board load/scoping/auto-refresh) by `board-scoping.test.ts` + the board page's 30s poll; Scenario 2 (drag-and-drop, invalid-transition rejection) by `drag-and-drop.spec.ts`'s 3 Playwright tests; Scenario 3 (filters, historical-name matching) by `filters.test.ts` + `board-reporting-api.test.ts`'s filter tests; Scenario 4 (audit trail) by `ticket-detail-audit.test.ts`; Scenario 5 (reports/export/RBAC) by `summary-report.test.ts` + `board-reporting-api.test.ts` + the new `board-to-report.spec.ts` e2e test. SC-001–SC-008 all pass; SC-005's one known gap (line-item removal has no actor/audit trail — a 004 schema limitation, not fixable from this feature) is documented in `lib/audit/consolidated-trail.ts`'s own comment rather than silently hidden.
- [X] T037 [P] E2E test for the full board-to-report flow in `tests/e2e/board-to-report.spec.ts`. Full lifecycle through a real browser: create store → activate it → create a catalogue part → create a ticket → confirm it appears on the Kanban board → move it through In Progress → apply the part → Complete it → generate today's report and confirm `totalTickets`/`partsRevenue` reflect it → download the CSV export. Passes alongside the other 10 existing e2e specs (11/11 green).
- [X] T038 [P] Accessibility audit (WCAG 2.1 AA) of the board (including keyboard drag-and-drop), filters, ticket detail view, and reports UI. Keyboard-only board operation already covered by `drag-and-drop.spec.ts` (Space to pick up, Arrow keys to move via the order-based `keyboard-coordinates.ts`, Space to drop) — the app's central WCAG requirement per `plan.md`'s `@dnd-kit` choice. All new reports UI form fields use explicit `<label htmlFor>`/`id` pairs (no unlabelled inputs); errors use `role="alert" aria-live="assertive"`, consistent with every other form in the app. No new ARIA landmarks/roles were introduced beyond the app's existing plain-semantic-HTML convention (headings, `<dl>`/`<dt>`/`<dd>`, `<table>`), so no regressions to the established baseline.
- [X] T039 [P] Security review: confirm board, filter, and export endpoints all resolve visibility through T006's single shared `ticket-query.ts` function rather than any ad hoc re-implementation of scoping (`research.md` §2's whole point). Confirmed by reading the three call sites: `app/api/tickets/route.ts` GET, `app/api/reports/export/route.ts`'s list-export branch, and (indirectly, for the summary/export-by-store path) `assertAccess` in `app/api/reports/summary/route.ts` and `export/route.ts`'s `type=summary` branch. No route reimplements `getScopedStoreIds`/store-id filtering independently — `queryScopedTickets` is the only place that intersects requested `storeIds` against the caller's scope.
- [X] T040 Performance check: board load and filter queries meet the constitution's <500ms p95, and confirm T004's `idx_tickets_customer_name` index is actually used by the Customer Name filter's query plan. Benchmarked in `scripts/bench-board-reporting-routes.ts` against 500 seeded tickets: unfiltered board load p95=10.7ms, `customerName` filter p95=4.1ms, `status` filter p95=5.8ms — all far under the 500ms budget. **Confirmed discrepancy from T006's flag**: `EXPLAIN` shows a plain `Seq Scan`, not an index scan — `idx_tickets_customer_name`'s plain btree index cannot accelerate a leading-wildcard `ILIKE '%value%'` query at all (only a trailing-wildcard prefix match or a trigram/GIN index could). At current and realistically-projected data volumes for a small-VPS single/few-store deployment (per the constitution's own target), the Seq Scan is still fast enough that this isn't an urgent problem, but the index as implemented is not actually earning its write-side cost for this filter — recommended follow-up: either switch to `pg_trgm` + a GIN index if contains-matching must stay, or drop the current btree index and accept the Seq Scan, rather than keeping a btree index that this query can never use.
- [X] T041 [P] Note the "first reached Completed" derived-fact check (`research.md` §1) as now triplicated across `004`'s Completed-lock, `005`'s once-only-notification trigger, and this feature's own `first-completed.ts` — a recommended follow-up refactor (a shared utility or SQL view) flagged for visibility, not silently accepted a third time. No new duplication added in Phase 7: `lib/reporting/summary.ts` computes its own "first Completed" via a `status_history` CTE (a 4th independent expression of the same underlying fact, inside raw SQL rather than as a call to `first-completed.ts`, since that helper returns one row at a time and the report needs it joined in bulk) — noting this here rather than silently letting the count grow un-flagged.
