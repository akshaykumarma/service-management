...
- [X] T043 [P] [US6] Contract tests for `GET`/`PATCH /api/templates/:type` and `POST /api/templates/:type/test-send` in `tests/contract/notifications-api.test.ts`
- [X] T044 [P] [US6] Integration test for template edit → `pending` approval state → unaffected live sends until approved → test-send preview → unsupported-placeholder rejection (FR-019) in `tests/integration/template-management.test.ts`. Approval itself is simulated by directly flipping `approval_status` in the DB, mirroring how Meta's own out-of-band review would eventually resolve it — nothing in this codebase triggers that transition itself (quickstart.md's own "Important" note).

### Implementation for User Story 6

- [X] T045 [US6] Implement template read/edit as an append-only insert (never update-in-place, so the previously-approved version stays retrievable), `approval_status` tracking, and allow-list validation at save time (`research.md` §5, FR-019) in `lib/whatsapp/templates.ts` (depends on T007). **Wiring beyond the task list**: `lib/notifications/send-completion.ts` and `lib/delivery/send-otp-message.ts` (both already implemented in earlier phases) were still reading the hardcoded `DEFAULT_TEMPLATE_BODY` constant directly — switched them to `getApprovedTemplateBody()` so an approved edit actually reaches `notifications.rendered_content`/the test-send preview; note this only affects the *recorded* text, since the real Meta API call is keyed by `type` against Meta's own pre-approved template structure either way (research.md §5), never by this body text.
- [X] T046 [US6] Implement `GET`/`PATCH /api/templates/:type` (returns current approved body plus any pending edit; `PATCH` sets `approval_status = 'pending'`, does not go live immediately) in `app/api/templates/route.ts` and `app/api/templates/[type]/route.ts` (depends on T045). `GET` is at `/api/templates` (both types at once); `PATCH` is per-type at `/api/templates/:type`, per the task's own file split.
- [X] T047 [US6] Implement `POST /api/templates/:type/test-send` (`usePending: true` renders the not-yet-approved wording directly rather than sending via Meta, since an unapproved template can't be sent outside a customer-initiated session — `contracts/notifications-api.md`) in `app/api/templates/[type]/test-send/route.ts` (depends on T045). **Interpretation**: `usePending: true` is pure preview (no WhatsApp call, no `notifications` row); `usePending` false/omitted actually sends the *approved* template for real and records a `notifications` row with `ticketId: null` — matching data-model.md's own stated reason that column is nullable ("null for a Super Admin test-send").
- [X] T048 [US6] Build the Super Admin template editor + test-send UI, making the pending-vs-approved distinction visible (FR-016's "new wording... subsequently sent" is true once Meta-approved, not on save) in `app/(dashboard)/admin/templates/page.tsx` (depends on T046-T047)
- [X] T049 [US6] Confirm T043-T044 pass; run `quickstart.md` Scenario 6 (depends on T045-T048). Passes; full suite (106 tests) green; added `tests/e2e/template-management.spec.ts` and verified the editor in a real browser (`next build && next start`) — edit, pending state, and preview all work; `npx tsc --noEmit` clean.
...
- [X] T050 [P] Run the full `quickstart.md` validation (all 6 scenarios, all 6 success-criteria checklist items) end-to-end. All 6 scenarios covered by passing tests (contract/integration/e2e) written across Phases 3-8. SC-001/002/003(partial, see T052 below)/004/005/006 all hold. Full suite: 106/106 vitest tests, 6/6 Playwright e2e specs, `npx tsc --noEmit` clean.
- [X] T051 [P] Accessibility audit (WCAG 2.1 AA) of the delivery/OTP UI and the template editor. Every input/select/textarea has an associated `<label>`; each delivery sub-state (start/verify/resend/locked/escalation) is its own `<section>`/`<div aria-labelledby>`; errors use `role="alert" aria-live="assertive"`, non-error status updates use `aria-live="polite"`. Same gap as every prior feature's own audit (`003`'s T031, `004`'s T029): no visual design/CSS exists yet, so color-contrast and focus-indicator criteria remain untested against real styling.
- [X] T052 [P] Security review: `otp_verifications.code_hash` never returned in any API response or logged in plaintext anywhere; webhook signature verification enforced; Admin/Super-Admin gates enforced on every escalation route (reinitiate, correct-phone, override). Verified: no route ever imports `otpVerifications` directly (all access goes through `lib/delivery/otp.ts`'s helpers, none of which expose `codeHash` or the plaintext code in a return value); `jobs/send-whatsapp-message.ts`'s error logging only logs the generic `WhatsAppSendError`, never the code; webhook route uses `timingSafeEqual`; `reinitiate`/`correct-phone`/`override` all call `requireAdminOrAbove` and are covered by `403` assertions in their respective tests. **Gap found and deliberately left open, flagged rather than silently fixed or ignored**: `003-ticket-lifecycle`'s own generic `PATCH /api/tickets/:id/status` endpoint still allows a direct, manual `toStatus: "delivered"` transition (its own pre-existing, tested behavior — `tests/integration/status-transitions.test.ts`'s "allows forward transitions..." case predates this feature and asserts exactly this). That means SC-003 ("every Delivered ticket has either `otp_verifications.verified_at` or a `delivery_overrides` row — never neither") is **not a strictly enforced invariant**: a Service Manager or Admin can still reach "Delivered" through the plain status endpoint with neither record. Closing this would mean changing `003`'s own contract and breaking its existing passing test, which is out of this feature's scope to decide unilaterally — recommend a `/speckit-clarify` pass on `003` and/or `005` to resolve the tension explicitly (either SC-003 is aspirational/describes the *intended* path rather than an absolute guarantee, or `003`'s generic endpoint needs an explicit carve-out for `toStatus: "delivered"`).
- [X] T053 Performance check: the synchronous parts of notification enqueue and OTP verification meet the constitution's <500ms p95 (the WhatsApp send itself is intentionally async, `research.md` §1). `scripts/bench-notification-routes.ts` (30 iterations, real route handlers, real Postgres): `PATCH /api/tickets/:id/status` → `completed` (the full notification-enqueue path, including `boss.send()`) p95 ≈ 16ms; `POST /api/tickets/:id/deliver/verify` (hash comparison + one UPDATE) p95 ≈ 6ms. Both comfortably under the 500ms target.
- [X] T054 [P] Note the "first reached Completed" derived-fact check (`research.md` §4) as the third independent implementation of the same pattern alongside `004-parts-services-catalogue`'s Completed-lock and `006-dashboard-reporting`'s report attribution — flagged there (`004`'s T032) as a future-consolidation candidate, not silently duplicated a third time without record. Done: `lib/notifications/send-completion.ts`'s `isFirstCompletion()` carries this exact note inline.

**Phase 9 note beyond the original task list**: while writing T053's benchmark, found and fixed a real bug — `jobs/send-whatsapp-message.ts` was previously invoking the WhatsApp Cloud API client with `params: {}`, meaning no template value (including the actual OTP code) was ever transmitted to Meta, only ever written to `notifications.rendered_content`; this had gone unnoticed since nothing before the Phase 5 e2e test actually inspected what the mock API received. Fixed during Phase 5 (see T029's note) by renaming the payload field to `templateParams` and threading it through. Also found that `instrumentation.ts` (Phase 2) never ran at all on this project's Next.js 14.x without `experimental.instrumentationHook: true` in `next.config.mjs` — fixed in the same pass. Both are noted here again since T053's benchmark is what would have caught the first bug's silent failure mode had it not already been found.
