...
- [X] T049 Post-v1 feature change, per direct product feedback (not in the original spec): admin-set passwords instead of a system-generated temporary one. `POST /api/auth/users` now requires a `password` field, checked against a shared complexity rule (`lib/auth/password-policy.ts`: 8+ chars, uppercase, lowercase, special character) — no more `temporaryPassword` in the response. Added `POST /api/auth/users/:id/reset-password` so a Super Admin can reset any staff member's password, and an Admin can reset a Store Service Manager's password within their own store(s) (broadened `GET /api/auth/users` to the same Admin scope, filtered to in-scope Service Managers only, so the UI has something to list). Both a reset and account creation immediately revoke the affected user's existing sessions, the same credential-rotation exception to FR-019 self-service reset already uses. Moved the staff-account UI from `/admin/users` (Super-Admin-only layout) to a new `/team` page reachable by both Admin and Super Admin, since Next.js's shared `(dashboard)/admin/layout.tsx` gate couldn't be loosened for just this one page without also exposing `/admin/stores`/`/admin/machine-models`/`/admin/catalogue` to Admins. `contracts/auth-api.md` updated to document both the deviation and the new endpoint. 8 new/updated contract tests (`tests/contract/auth-api.test.ts`) plus the existing `user-provisioning.test.ts` integration test updated to log in with the admin-chosen password.
