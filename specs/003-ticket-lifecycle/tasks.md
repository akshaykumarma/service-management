...
- [X] T030 [P] Ran the full `quickstart.md` validation via the automated suite: SC-001-SC-008 each map to a named test (SC-001/SC-002 → `ticket-intake-history.test.ts` + `tickets-api.test.ts`; SC-003 → `ticket-number-collision.test.ts`; SC-004/SC-007 → `status-transitions.test.ts`; SC-005 → `status-transitions.test.ts`'s actor/timestamp assertion; SC-006 → `holds-cancellations.test.ts`; SC-008 → `concurrent-status-change.test.ts`). 60/60 tests passing.
- [X] T031 [P] Accessibility audit (WCAG 2.1 AA) of the intake and ticket detail pages: every input/select/textarea has an associated `<label>`, status/history sections use `aria-labelledby` headings, errors use `role="alert" aria-live="assertive"`. Same gap as `002-auth-rbac`'s own audit: no visual design/CSS exists yet, so color-contrast and focus-indicator criteria are untested against real styling — deferred to the same future design-system pass.
- [X] T032 [P] Security review — **found and fixed two real gaps**: (1) photo content-type/size were validated only at presigned-URL issuance, never re-checked against what was actually uploaded — research.md §5 explicitly calls for this and it was missing. A presigned PUT URL constrains the signed Content-Type but not body size at all, so a client could still upload an oversized file. Added `verifyUploadedObject()` (`lib/tickets/photos.ts`), called before every ticket creation with photos: a `HeadObjectCommand` re-validates the real content-type/size, deleting the object and rejecting the request on mismatch. (2) Found while fixing (1): the ticket-creation code was hard-coding `contentType: "image/jpeg", sizeBytes: 0` into `ticket_photos` rows instead of persisting the real uploaded values — fixed to use `verifyUploadedObject()`'s actual metadata. Both fixes verified against a real upload through `s3rver` (`tests/integration/photo-verification.test.ts`), not mocked. Presigned URL expiry (300s) and RBAC (`assertAccess`/CSRF) on every state-changing route confirmed by inspection and existing tests.
- [X] T033 Performance check (`scripts/bench-ticket-routes.ts`, 30 iterations against real route handlers, real Postgres, 200 seeded rows for the history-lookup query to scan): `POST /api/tickets` (including the inline history lookup) p95 ≈ 9ms. Comfortably under the constitution's 500ms p95 target.
- [X] T034 [P] The `stores` table cross-feature coordination note from T004 resolved itself by build order (`002-auth-rbac` was implemented first and created the table `003` needed) — documented as a completed fact in T004 rather than a forward-looking warning, since there's no longer an open coordination risk to document for future implementers.
